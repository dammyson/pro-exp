...
class RevenueController extends Controller
{
    ...
    public function getDailyRevenueByCampaign($campaign_id)
    {
        try {
            ...
            $revenueAds = RevenueHistory::where('campaign_id', $campaign_id)
                            ->where('category', 'ads')->distinct('activity_id')
                            ->whereDate('created_at', Carbon::now()->toDateString())
                            ->count();

            $revenueSubscriptions = RevenueHistory::where('campaign_id', $campaign_id)
                                        ->where('category', 'subscription')->distinct('activity_id')
                                        ->whereDate('created_at', Carbon::now()->toDateString())
                                        ->count();
