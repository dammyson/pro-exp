...
class CampaignGamePlayPurchaseController extends Controller
{
    ...
    */
    ...
    public function index($campaign_id, $audience_id) 
    {
    ...
    /**
     * show campaign purchase by audience
     * 
    ...
    */
    ...
    {
        try {
            $subscriptions = CampaignSubscription::where('campaign_id', $campaign_id)->where('audience_id', $audience_id)->latest()->first();
        ...
        return response()->json(['error' => false, 'message' => 'Campaign game play purchase retrieved by audience ID', 'data' => $subscriptions], 200);
    }
    ...
    public function show($campaign_id, $id)
    {
        try {
            ...
            // report($th);
            return response()->json(['error' => true, 'message' => $throwable->getMessage()]);
        }
    ...
    */
    ...
    {
        try {
            $subscription = CampaignGamePlayPurchase::where('campaign_id', $campaign_id)->where('audience_id', $audience_id)->first();
        ...
        return response()->json([
            'error' => false,
            'message' => 'Campaign game play purchase retrieved by audience ID', 'data' => $subscription
        ], 200);
    ...
    public function freeGamePlays(Request $request, $campaign_id)
    {
        ...
        try {
            ...
            if ($game_rule['has_free_game_play'] == false) {
                return response()->json([
                    'error' => false,
                    'message' => 'campaign does not have free game play',
                    'data' => []
                ], 400);
            } 
            // check subscription plans for a free plan
            $free_subscription = CampaignSubscriptionPlan::where('campaign_id', $campaign_id)
                ->where('price', 0.0)->first();
            ...
            // check if audience as already been allocated free subscription
        ...
        return response()->json([
            'error' => false,
            'message' => 'You have been allocated free game plays for this campaign', 
