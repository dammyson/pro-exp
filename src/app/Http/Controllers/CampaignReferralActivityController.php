...
class  CampaignReferralActivityController extends Controller
{
    ...
    public function create(Request $request, $campaign_id)
    {
        ...
        try {
            ...
            if ($referral == null) {
                return response()->json(['error' => true, 'message' => 'Invalid referrral code'], 422);
            }
            
            //this means if the campaignId or referentId(audience) are different from what is in the db
            // we can have the same referralId used for them
        ...
        try {
            ...
            DB::transaction(function () use ($referents){
                foreach ($referents as $referent) {
                    ...
                    // $campaignActivity =CampaignReferralActivity::where('id', $referent->id)->first();
                    // $campaignActivity->is_activation_point_redeemed = 1;
                    // $campaignActivity->save();
                }
            });
