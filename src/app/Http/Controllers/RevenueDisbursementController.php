...
class RevenueDisbursementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            DB::transaction(function () use($validated) {
                ...
                // disburse group 1 -> plaform owner
                ...
                $this->createHistory(
                    $platformOwnerwallet->id, 
                    $this->computeRevenue(50, $validated['revenue_type'], $validated['revenue']), 
                    $validated['revenue_type'], 
                    $validated['channel'],
                ...
                $this->createHistory(
                    $partnerWallet->id, 
                    $this->computeRevenue(20, $validated['revenue_type'], $validated['revenue']), 
                    $validated['revenue_type'], 
                    $validated['channel'],
                    $validated['campaign_id'],
                    $validated['activity_id']
                ...
                if (count($batchAudience[0]) > 0 && $batchAudience[0]['referrer_id'] !== null) {
                    ...
                    $this->createHistory(
                        $defaultInfluencerWallet->id, 
                        $this->computeRevenue(5, $validated['revenue_type'], $validated['revenue']), 
                        $validated['revenue_type'], 
                        $validated['channel'],
                        $validated['campaign_id'],
                        $validated['activity_id']
                ...
                // disburse influencer. check that influencer is the same as audience
                ...
                if ($validated['influencer_id'] == null) {
                    ...
                    $this->createHistory(
                        $platformOwnerwallet->id, 
                        $this->computeRevenue(25, $validated['revenue_type'], $validated['revenue']), 
                        $validated['revenue_type'], 
                        $validated['channel'],
                        $validated['campaign_id'],
                        $validated['activity_id']
                ...
                } else if ($validated['influener_id'] != $validated['audience_id']) {
    ...
    public function computeRevenue($percent, $revenue_type, $revenue_amount)
    {
        ...
        } else if ($revenue_type == 'ads') {
            $revenue = 0.7; // 70 kobo per ad view;
        }

        // 80% revenue is split by stakeholders
